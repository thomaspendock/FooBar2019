def is_path(graph, src, sink, parent):
    rows = len(graph)

    # all nodes are not visited except src
    visited = [True if i == src else False for i in range(rows)]
    queue = [src]

    while queue:
        node = queue.pop(0)
        # get all adjacent nodes from current and
        for i, val in enumerate(graph[node]):
            if visited[i] or val <= 0: continue
            queue.append(i) # queue adjacent if not visited
            visited[i] = True
            parent[i] = node
            
    return visited[sink]

def max_flow(graph, src, sink, flow=0):
    parent = [-1] * len(graph)

    # add flow while there is path from src to sink
    while is_path(graph, src, sink, parent):
        ...
        # update the new capacities of edges (subtract path just taken)
        cur = sink
        while cur != src:
            par = parent[cur]
            graph[par][cur] -= path_flow
            graph[cur][par] += path_flow
            cur = par # retrace steps backward
            
    return flow

def solution(entr, exits, paths):
    width = len(paths[0])+2
    big_num = 2**20

    # add 1 because we are adding a row and column to graph
    exits = [e+1 for e in exits]
    entr  = [e+1 for e in entr]

    # source and sink row for graph
    src  = [(big_num if i in entr else 0) for i in range(width)]
    sink = [0 for i in range(width)]

    # create new graph with source and sink
    graph = [src]
    for i in range(1, len(paths)+1):
        # new column  + orig row   + path to sink
        new_row = [0] + paths[i-1] + ([big_num] if i in exits else [0])
        graph.append(new_row)
    graph.append(sink)
    
    return max_flow(graph, 0, len(graph)-1)
...
x = solution(entrances, exits, path)
print(x)
